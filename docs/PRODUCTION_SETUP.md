...
The setup wizard guides you through the entire process interactively.
...
6. Output your gateway URL

### Step 3: Verify Deployment

```bash
# The wizard outputs your URL, e.g.:
# https://unitone-agentgateway-abc123.azurecontainerapps.io

# Check UI
open https://YOUR_URL/ui
...
Enable Azure AD authentication for your gateway:

```bash
# Get your Container App resource ID
...
az acr build \
  --registry $ACR_NAME \
  --image unitone-agentgateway:$(git rev-parse --short HEAD) \
  --image unitone-agentgateway:latest \
  --file Dockerfile.acr \
  .
