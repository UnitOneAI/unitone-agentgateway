...
resource "azurerm_container_app" "app" {
  ...
  # ACR registry credentials (always include so updates can use ACR images)
  registry {
  ...
  template {
    ...
    container {
      name   = "agentgateway"
      image  = var.use_placeholder_image ? var.placeholder_image : "${azurerm_container_registry.acr.login_server}/unitone-agentgateway:${var.image_tag}"
      cpu    = 1.0
      memory = "2Gi"
...
# ==============================================================================
# 10b. Client Certificate Mode (mTLS)
# ==============================================================================
# Configure client certificate mode for mTLS authentication.
# Modes: ignore (default), accept, require
resource "null_resource" "client_certificate_mode" {
  count = var.client_certificate_mode != "ignore" ? 1 : 0
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      az containerapp ingress update \
        --name ${local.container_app_name} \
        --resource-group ${var.resource_group_name} \
        --client-certificate-mode ${var.client_certificate_mode}
    EOT
  }
