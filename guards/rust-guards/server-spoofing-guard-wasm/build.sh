#!/usr/bin/env bash
#
# Build script for Server Spoofing Guard (Rust) WASM component
#
...
echo "Building Server Spoofing Guard (Rust) WASM component..."

# Determine the correct WASM target name
# Newer Rust versions renamed wasm32-wasi to wasm32-wasip1
...
wasm-tools component new \
    "target/${WASM_TARGET}/release/server_spoofing_guard_wasm.wasm" \
    --adapt "$ADAPTER_FILE" \
    -o server-spoofing-guard-wasm.component.wasm
...
if command -v wasm-opt &> /dev/null; then
    ...
    wasm-opt -Oz \
        ...
        server-spoofing-guard-wasm.component.wasm
    rm server-spoofing-guard-wasm.component.wasm
else
    mv server-spoofing-guard-wasm.component.wasm server_spoofing_guard.wasm
fi

size=$(wc -c < server_spoofing_guard.wasm | awk '{print int($1/1024)"KB"}')
echo "Build complete: server_spoofing_guard.wasm ($size)"
